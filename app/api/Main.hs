...
instance FromHttpApiData MyDay where
    parseQueryParam x = parsingError $ MyDay <$> parseUTC x
        where parseUTC :: T.Text -> Maybe UTCTime
...
parsingError :: Maybe MyDay -> Either T.Text MyDay
parsingError Nothing  = Left "parsing error"
parsingError (Just x) = Right x
